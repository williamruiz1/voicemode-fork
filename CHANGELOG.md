...
- **Natural mode — barge-in turn-taking, Phase 1** — A second talk mode alongside today's turn-taking ("turn mode", unchanged, still the default): the mic stays hot WHILE TTS is playing, so speaking over the agent instantly halts playback mid-word and becomes the next turn, without a re-prompt. Toggle: `convomode-natural-mode.sh on|off|toggle|status` (writes/removes `~/.voicemode/natural-mode.flag`; no server restart needed). Under the hood — a concurrent mic `InputStream` runs alongside TTS playback; a lightweight NLMS software echo canceller (`voice_mode/aec.py`) cancels the agent's own voice out of the mic signal using the exact reference audio being played, so the mic hearing itself doesn't false-trigger; on sustained post-cancellation speech (default 300ms, tunable) the existing mid-buffer instant-stop mechanism (previously built for the manual Pause flag) halts TTS, and the audio already captured seeds the next turn's recording. Fully inert in turn mode -- absent the flag file, converse() behaves byte-for-byte as before. New tunables (all optional, safe defaults, unset = off/default behavior): `VOICEMODE_BARGE_IN_TRIGGER_MS`, `VOICEMODE_BARGE_IN_VAD_AGGRESSIVENESS`, `VOICEMODE_AEC_FILTER_MS`, `VOICEMODE_AEC_REF_DELAY_MS`, `VOICEMODE_AEC_STEP_SIZE`. **Live on-device tuning against a real microphone (delay compensation, AEC step size, VAD threshold) has NOT been done yet** -- the mechanism is validated against synthetic echo signals only (see `tests/test_aec.py`, `tests/test_barge_in.py`); see `docs/guides/natural-mode.md` for the trial instructions and what to retune if it over/under-triggers.
...
- **A queued agent no longer barges into another agent's live conversation the instant it opens the mic** (founder-os#13445-conch-barge-in, 2026-08-04) — The conch's yieldable-listen mechanism (vibedispatcher#132) let a waiter's `wait_for_conch=true` poll loop preempt the current holder on the very first tick of a fresh idle-listen (`recording_duration == 0`), because `Conch.request_yield()` could already be "fresh" from polling that started *before* the holder's TTS even finished. In practice: agent A finishes speaking, opens the mic for the human's reply, and is preempted after ~0.1–0.2s with zero samples captured — the human never gets a chance to respond before agent B's TTS cuts in over what was still an active conversation. Added `VOICEMODE_CONCH_YIELD_GRACE_SECONDS` (default `3`): a fresh idle-listen must run for at least this long before a yield request is honored. Set to `0` to restore the old zero-grace behavior. TTS is still never preempted mid-utterance (unchanged); this only affects the idle-listen yield path.
